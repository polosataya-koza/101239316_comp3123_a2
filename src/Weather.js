...
const Weather = () => {
    ...
    return (
        <div className="main-container">
            <button type="button" className="button-click" onClick={fetchWeather}>Tap to see new info about the weather in Toronto</button><br/>
            {weather.main && (
                <div className="city">
                    <h2 className="city-name"><span>{weather.name}, {weather.sys.country}</span></h2>
                    <h3 className="city-name-med">{months[(date.getMonth()).toLocaleString()]} {date.getDate()}, {days[date.getDate()]}</h3>
